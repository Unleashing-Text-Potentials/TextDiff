...
from src.modeling.Diffmodel import StochasticText
from src.modeling.Diffmodel_mean import StochasticText_mean
from src.modeling.Stage2ConfidenceModulator import Stage2ConfidenceModulator
from src.modeling.AdaptiveWeightedAverage import AdaptiveWeightedAverage
...
class CLIPOutput(ModelOutput):
    ...
    text_embeds1: torch.FloatTensor = None
    image_embeds1: torch.FloatTensor = None
    text_embeds2: torch.FloatTensor = None
    image_embeds2: torch.FloatTensor = None
    conf_weights: torch.FloatTensor = None
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.stochastic = StochasticText()
        
        self.stochastic_mean = StochasticText_mean()

        self.stage2_confidence_modulator = Stage2ConfidenceModulator()

        self.aggregator  = AdaptiveWeightedAverage()
    ...
    def forward(
        ...
        text_embeds1 = self.stochastic(text_features = text_embeds , images = image_tensor)

        text_embeds_mean = self.stochastic_mean(text_features = text_embeds , images = image_tensor)

        text_embeds_mean = self.aggregator(text_embeds_mean)

        conf_weights = self.stage2_confidence_modulator( text_embeds_mean , text_embeds1)

        # normalized features
        image_embeds = image_embeds / image_embeds.norm(dim=-1, keepdim=True)
        text_embeds1 = text_embeds1 / text_embeds1.norm(dim=-1, keepdim=True)
        ...
        return CLIPOutput(
            ...
            image_embeds1=image_embeds,
            text_embeds2=text_embeds_mean,
