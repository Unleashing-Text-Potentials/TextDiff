...
class HDVILAVideoRetrievalDataset(Dataset):
    ...
    def __init__(self, cfg, vis_dir, anno_path, vis_format='video', mode="train"):
        ...
        if "anet" in self.anno_path:
            text_image_path = "blob_mount/clip_data/vis_db/diff_data/anet/" + self.mode + ".json"
            diff_number = 10
            diff_image = "/diff-image/anet"
        
        if "didemo" in self.anno_path:
            text_image_path = "blob_mount/clip_data/vis_db/diff_data/didemo/" + self.mode + ".json"
            diff_number = 10
            diff_image = "/diff-image/didemo"
        
        if "msrvtt" in self.anno_path:
            text_image_path = "blob_mount/clip_data/vis_db/diff_data/msrvtt/" + self.mode + ".json"
            diff_number = 5
            diff_image = "/diff-image/msrvtt"

        if "lsmdc" in self.anno_path:
            text_image_path = "blob_mount/clip_data/vis_db/diff_data/lsmdc/" + self.mode + ".json"
            diff_number = 5
            diff_image = "/diff-image/lsmdc"
        
        self.text_image_path = load_json(text_image_path)
        self.diff_number = diff_number
    ...
    def id2path(self, id):
        ...
        if self.vis_format == 'video':
            name = os.path.join(self.vis_dir, clip_name.split('/')[-1])
            if "msrvtt" in self.vis_dir:
                name = os.path.join(self.vis_dir, clip_name.split('/')[-1]+".mp4")
            if "lsmdc" in self.vis_dir:
                clip_prefix = clip_name.split('.')[0][:-3]
                name = os.path.join(self.vis_dir, clip_prefix , clip_name + ".avi")
        else:
    ...
    def __getitem__(self, index):
        ...
        if isinstance(texts, list):
            ...
            if 'didemo' in self.anno_path or 'anet' in self.anno_path:
        ...
        image_path = []
        image_name = self.text_image_path[ texts[0] ]
        for i in range(self.diff_number):
            image_path_n = self.diff_image+ "/" + str(i) + "/" + self.mode +"/" + str( image_name ) +'.pt'
            image_path.append(image_path_n)
        all_image_tensor =  []

        for path in image_path:
            image_tensor = torch.load(path ,  map_location="cpu" ).to(torch.float32)
            all_image_tensor.append( image_tensor )
        
        all_image_tensor = torch.stack(all_image_tensor).float()

        return dict(
            video = video,  # [clips*num_frm, C, H_crop, W_crop]
            texts = texts,
            image_tensor = all_image_tensor
        )
...
class VideoRetrievalCollator(object):
    ...
    def collate_batch(self, batch):
        ...
        image_tensors = default_collate([d["image_tensor"] for d in batch])
