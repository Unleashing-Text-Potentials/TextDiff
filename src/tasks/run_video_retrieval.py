...
def validate(model, val_loaders, cfg):
    ...
    for loader_name, val_loader in val_loaders.items():
        ...
        for val_step, batch in enumerate(val_loader):
            ...
            vis_feat = hvd.allgather(feats['vis_features1'])
            text_feat = hvd.allgather(feats['text_features1'])
            vis_feat2 = hvd.allgather(feats['vis_features2'])
            text_feat2 = hvd.allgather(feats['text_features2'])
            conf_weight = hvd.allgather(feats['conf_weights']) 

            # print('allgather vis_features', vis_feat.shape)

            text_feats.append(text_feat.cpu().numpy())
            vis_feats.append(vis_feat.cpu().numpy())
            text_feats2.append(text_feat2.cpu().numpy())
            vis_feats2.append(vis_feat2.cpu().numpy())
            conf_weights.append(conf_weight.cpu().numpy())
        ...
        text_feats2 = np.vstack(text_feats2)
        vis_feats2 = np.vstack(vis_feats2)
        conf_weights = np.vstack(conf_weights)

        text_feats = text_feats[:valid_len]
        vis_feats = vis_feats[:valid_len]
        text_feats2 = text_feats2[:valid_len]
        vis_feats2 = vis_feats2[:valid_len]
        conf_weights = conf_weights[:valid_len]

        sim_matrix1 = cal_cossim(text_feats, vis_feats)
        sim_matrix2 =  cal_cossim(text_feats2, vis_feats2)

        sim_matrix2 = conf_weights*sim_matrix2

        sim_matrix = sim_matrix2 + sim_matrix1
        for type in ["simple" ]:
    ...
    model.train()
    return val_log, t2vr1 , sim_matrix
def start_training():
    ...
    for step, batch in enumerate(InfiniteIterator(train_loader)):
        ...
        if cfg.loss_config.if_gather:
            vis_feat1 = hvd.allgather(outputs['vis_features1'])
            text_feat1 = hvd.allgather(outputs['text_features1'])
            vis_feat2 = hvd.allgather(outputs['vis_features2'])
            text_feat2 = hvd.allgather(outputs['text_features2'])
            conf_weights = hvd.allgather(outputs['conf_weights'])
            if cfg.loss_config.loss_name in ["NCELearnableTempLoss", "NCELearnableTempDSLLoss"]:
                ...
                loss = loss_func(vis_feat1, text_feat1, vis_feat2 , text_feat2 , conf_weights , logit_scale)
            else:
                loss = loss_func(vis_feat1, text_feat1)
        else:
            loss = outputs['loss']
        ...
        if (step + 1) % cfg.gradient_accumulation_steps == 0:
            ...
            if global_step % cfg.valid_steps == 0:
                ...
                _, t2vr1 , sim_matrix= validate(model, inference_loaders, cfg)
                ...
                if hvd.rank() == 0 and cfg.if_model_saver and t2vr1 > best_model_saver.bestr1:
                    ...
                    np.save("tv_sim_matrix.npy", sim_matrix)

                LOGGER.info(
                        f"t2v recall@1: {best_model_saver.bestr1 } " )
            else:
