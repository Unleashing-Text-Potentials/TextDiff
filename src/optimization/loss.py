...
class NCELearnableTempLoss(nn.Module):
    ...
    def forward(self, vis_feat1, text_feat1, vis_feat2 , text_feat2 , conf_weights , temp):
        logit_scale = temp.exp()

        t2v1 = torch.matmul(vis_feat1, text_feat1.permute(1, 0)) * logit_scale

        t2v2 = torch.matmul(vis_feat2, text_feat2.permute(1, 0)) * logit_scale
    
        

        t2v = conf_weights * t2v2 + t2v1
        loss1 = self.get_loss(t2v)
        loss2 = self.get_loss(t2v1)
        loss3 = self.get_loss(t2v2)

        return loss1 + loss2 + loss3*0.1
